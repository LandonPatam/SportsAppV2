                        ...
                        >
                          <CardHeader className="pb-3">
                            <div className="flex items-start justify-between">
                              <div>
                                <div className="flex items-center gap-2 mb-2">
                                  <Badge variant="default" style={{ backgroundColor: "#3b018eff", color: "#ffffffff" }}>Circuit {mainRace.round}</Badge>
                                  {hasSprint && (
...
const trackPaths: Record<string, string> = {
  ...
  3 :"M6.2992 680.89c-0.27206 2.7661-0.32461 5.9077-0.28875 9.5448 0.67564 68.534 60.805 41.227 118.7-53.906 16.87-27.72 21.561-35.516 68.076-141.97 18.84-43.117 23.026-56.997 23.274-56.976-17.092 1.192-25.383 2.2997-37.965 4.0064-48.895 6.6326-47.583 5.5874-71.913-7.2867-22.194-11.744-29.63-11.717-29.052 0.10395 0.2816 5.7586 1.51 6.9693 23.815 23.484 42.099 31.17 48.486 42.761 45.308 82.227-4.4455 55.202-33.953 89.307-95.079 109.89-35.492 11.953-43.405 15.942-44.875 30.879z M142.18 223.44c-0.99608 11.693 2.884 22.54 11.588 33.888 5.1986 6.7772 4.9363 9.7098-1.4371 16.069-10.814 10.79-11.631 8.9266 24.278 55.369 27.341 35.361 29.452 38.603 35.987 55.27 9.2469 23.585 10.571 36.172 5.1053 48.577l-1.9052 5.0575c6.9225-0.27444 13.733-0.99567 19.259-1.5917 20.642-2.2211 20.032-1.0905 19.849-36.816-0.17303-33.687 1.7921-26.874-24.771-52.077-43.259-41.045-53.044-67.407-38.766-104.44 14.669-38.051 28.702-44.956 61.769-30.393 34.957 15.396 52.776 6.0688 57.767-30.238 3.529-25.671 10.926-32.681 38.921-36.883 25.27-3.7932 32.188-10.1 35.215-32.108 2.7446-19.953 2.7019-19.911 41.042-40.153 27.76-14.672 32.96-18.198 36.85-25.028 11.21-19.646-6.55-39.166-38.09-41.857-12.5-1.0664-19.21 3.0634-121.87 74.955-15.842 11.094-31.26 21.767-34.262 23.719-59.125 38.448-103.97 72.264-112.46 84.812-8.5034 12.559-13.178 23.535-14.057 33.853z", // 🏎️ just an example — paste Monaco’s path here
...
const trackStyles: Record<string, React.CSSProperties> = {
  1: {
  ...
  3: {
    transform: "scale(0.5) translate(160%, -120%) rotate(60deg)",
  },
  4: {
...
const NeonTrack = ({ circuitName, circuitId }: NeonTrackProps) => {
  const id = circuitId ?? null;
  const path = (id && trackPaths[id]) || trackPaths[1]; // fallback to circuit #1 (e.g. Bahrain)
      ...
      >
        <defs>
          <linearGradient id="neonGradient">
            <stop offset="0%" stopColor="#5900b7ff" />
            <stop offset="50%" stopColor="#ffffffff" />
            <stop offset="100%" stopColor="#5900b7ff" />
            <animateTransform
